<template>
  <div class="app-container">
    <el-form :inline="true" class="demo-form-inline">
      <el-form-item label="评价学生">
        <el-select v-model="dataVo.categoryId" placeholder="请选择评价学生">
          <el-option
      ...
      <el-form-item label="评价课程">
        <el-select v-model="dataVo.courseId" placeholder="请选择评价课程">
          <el-option
    ...
    >
      ...
      <el-table-column prop="courseName" label="评定课程名称" />
      <el-table-column prop="userName" label="评定学生姓名" />
    ...
    >
      <el-form :model="formData" label-position="left" label-width="120px">
        ...
        <el-form-item label="评价学生" v-if="disabledVisible">
          <el-select filterable style="width:100%" v-model="formData.appointmentId" placeholder="请选择评价学生">
            <el-option
              v-for="question in scoreList"
              :key="question.id"
              :label="`学生姓名:${question.userName}， 评定课程：${question.courseName}`"
              :value="question.id"></el-option>
          </el-select>
        </el-form-item>
...
<script>
import { deleteById, insert, update, pageQuery, getNotScoreInfo } from '@/api/score'
...
export default {
  ...
  methods: { // 创建具体的方法, 定义的方法
    ...
    getScoreList(){
      getNotScoreInfo().then(res=>{
        this.scoreList = res.data.row
      })
    },
